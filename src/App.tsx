...
const Header = ({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: (t: string) => void }) => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 pointer-events-none">
      ...
      <div className={`bg-[#051a05] text-white/70 py-2 text-[10px] font-bold uppercase tracking-[0.25em] transition-all duration-700 pointer-events-auto ${
        scrolled ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'
      }`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2 group cursor-pointer">
              <Phone size={12} className="text-green-500 group-hover:scale-110 transition-transform" />
            ...
            <div className="hidden sm:flex items-center gap-2 group cursor-pointer">
              <MapPin size={12} className="text-green-500 group-hover:scale-110 transition-transform" />
              <span className="group-hover:text-white transition-colors">Thủ Đức, TP. Hồ Chí Minh</span>
            </div>
          </div>
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4 border-l border-white/10 pl-6">
              <button className="hover:text-green-400 transition-colors">VN</button>
              <button className="hover:text-green-400 transition-colors opacity-40">EN</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`w-full transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] pointer-events-auto ${
        scrolled ? 'pt-4' : 'pt-0'
      }`}>
        <div className={`mx-auto transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          ...
          <div className={`relative transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
            scrolled 
              ? 'bg-white/80 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] rounded-[32px] border border-white/40 py-3 px-6' 
            ...
            <div className="flex justify-between items-center">
              ...
              >
                <div className={`relative transition-all duration-500 ${scrolled ? 'w-10 h-10' : 'w-14 h-14'} bg-white rounded-[18px] flex items-center justify-center shadow-xl group-hover:shadow-green-600/20 group-hover:rotate-3 overflow-hidden border border-green-50`}>
                  <img 
                    src="/logo.png" 
                    alt="TRẠM XANH Logo" 
                ...
                <div className="ml-4">
                  <h1 className={`font-black text-green-950 leading-none tracking-tighter transition-all duration-500 ${scrolled ? 'text-[18px]' : 'text-[24px]'}`}>TRẠM XANH</h1>
                  <p className={`text-green-600 font-bold uppercase tracking-[0.3em] transition-all duration-500 ${scrolled ? 'text-[8px] mt-0.5' : 'text-[10px] mt-1.5'}`}>Nông Sản Việt</p>
                </div>
              </motion.div>
              ...
              <nav className="hidden lg:flex items-center gap-1">
                {navItems.map((item) => (
                  <button
                    ...
                    className={`relative px-5 py-2.5 text-[13px] font-bold uppercase tracking-widest transition-all duration-500 rounded-xl group ${
                      activeTab === item.id 
              ...
              <div className="hidden md:flex items-center gap-3">
                <div className="flex items-center bg-gray-100/50 rounded-2xl p-1 mr-2">
                  <button className="p-2.5 text-gray-500 hover:text-green-600 hover:bg-white rounded-xl transition-all">
                    <Search size={18} />
                  </button>
                  <button className="p-2.5 text-gray-500 hover:text-green-600 hover:bg-white rounded-xl transition-all relative">
                    <ShoppingCart size={18} />
                    <span className="absolute top-2 right-2 w-2 h-2 bg-green-600 rounded-full border-2 border-white"></span>
                  </button>
                </div>
                <motion.button 
                  whileHover={{ scale: 1.05, boxShadow: '0 15px 30px -10px rgba(22,163,74,0.5)' }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() => setActiveTab('products')}
                  className="bg-green-600 text-white px-7 py-3 rounded-2xl text-[13px] font-bold shadow-lg shadow-green-600/20 transition-all flex items-center gap-2"
                >
                  <span>MUA NGAY</span>
                  <ArrowRight size={14} />
                </motion.button>
              ...
              <div className="lg:hidden flex items-center gap-3">
                <button className="p-2.5 text-gray-500 bg-gray-100/50 rounded-xl">
                  <ShoppingCart size={20} />
                </button>
                <button 
                  onClick={() => setIsMenuOpen(!isMenuOpen)} 
                  className="p-3 rounded-xl bg-green-600 text-white shadow-lg shadow-green-600/20"
                >
        ...
        {isMenuOpen && (
          <>
            <motion.div
              ...
              className="fixed inset-0 bg-[#051a05]/40 backdrop-blur-md z-[-1] pointer-events-auto"
            />
            <motion.div
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              ...
              className="absolute top-0 right-0 h-screen w-full max-w-[320px] bg-white shadow-[-20px_0_60px_rgba(0,0,0,0.1)] z-50 pointer-events-auto flex flex-col"
            >
              <div className="p-8 flex flex-col h-full">
                <div className="flex justify-between items-center mb-12">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-black text-green-950 text-xl tracking-tighter">TRẠM XANH</span>
                  </div>
                  <button onClick={() => setIsMenuOpen(false)} className="p-2 text-gray-400 hover:text-green-600 transition-colors">
                    <X size={24} />
                  </button>
                </div>
                ...
                <nav className="space-y-2 mb-12">
                  {navItems.map((item, i) => (
                    <motion.button
                      ...
                      className={`flex items-center justify-between w-full px-6 py-5 rounded-2xl text-[16px] font-bold transition-all ${
                        activeTab === item.id 
                ...
                <div className="mt-auto space-y-6">
                  <div className="p-6 bg-green-50 rounded-[24px] border border-green-100 relative overflow-hidden group">
                    <div className="absolute -right-4 -bottom-4 opacity-5 group-hover:scale-110 transition-transform duration-700">
                      <Phone size={80} />
                    </div>
                    <p className="text-[11px] text-green-800 font-black uppercase tracking-widest mb-2">Hotline hỗ trợ</p>
                    <p className="text-[20px] text-green-700 font-black">0223 223 331</p>
                  </div>
                  <motion.button 
                    whileTap={{ scale: 0.95 }}
                    className="w-full bg-green-950 text-white py-5 rounded-[24px] font-bold shadow-2xl shadow-green-950/20 flex items-center justify-center gap-3"
                  >
